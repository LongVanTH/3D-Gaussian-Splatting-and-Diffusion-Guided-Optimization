...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ###
            cov_3D = torch.eye(3).unsqueeze(0).repeat(len(scales), 1, 1).to(scales.device)
            cov_3D = cov_3D * (scales**2).unsqueeze(-1)  # (N, 3, 3)
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            cov_3D = torch.bmm(torch.bmm(R, S), R.transpose(1, 2))  # (N, 3, 3)
    ...
    def compute_cov_2D(
        ...
        JW = torch.bmm(J, W)
        cov_2D = torch.bmm(JW, torch.bmm(cov_3D, JW.transpose(1, 2)))  # (N, 2, 2)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        means_camera_space = camera.transform_points(self.gaussians.means)
        # Inverse depth because perspective camera, see https://pytorch3d.org/docs/cameras
        z_vals = 1 / means_camera_space[:, 2]  # (N,)

        # R, T = camera.R[0], camera.T[0]
        # means_view_space = self.gaussians.means @ R + T
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        """
        ### YOUR CODE HERE ###
        valid_idxs = torch.where(z_vals >= 0.0)[0]
        sorted_idxs = torch.argsort(z_vals[valid_idxs])
